...
-- CREATE INDEX IF NOT EXISTS idx_paymentprocessing_status ON paymentprocessings (status);

-- Created date (pagination/sorting by date)
-- CREATE INDEX IF NOT EXISTS idx_paymentprocessing_created_at ON paymentprocessings (created_at DESC);

-- Composite: status + created_at (covers most list queries)
-- CREATE INDEX IF NOT EXISTS idx_paymentprocessing_status_created
--     ON paymentprocessings (status, created_at DESC);

-- Foreign key index (prevents full-table scan on joins)
-- CREATE INDEX IF NOT EXISTS idx_paymentprocessing_user_id ON paymentprocessings (user_id);
