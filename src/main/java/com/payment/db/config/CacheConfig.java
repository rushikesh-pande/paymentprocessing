 ...
 *
 * Usage in service:
 *   @Cacheable(value = "paymentprocessingCache", key = "#id")
 *   @CacheEvict(value = "paymentprocessingCache", key = "#id")
 *   @CachePut(value  = "paymentprocessingCache", key = "#result.id")
 */
@Configuration
@EnableCaching
public class CacheConfig {
    ...
    public CacheManager cacheManager(RedisConnectionFactory factory) {
        ...
        // ── paymentprocessing specific caches ─────────────────────────────────────
        caches.put("paymentprocessingCache",       defaultConfig().entryTtl(MEDIUM));
        caches.put("paymentprocessingListCache",   defaultConfig().entryTtl(SHORT));
        caches.put("paymentprocessingCountCache",  defaultConfig().entryTtl(SHORT));
        caches.put("paymentprocessingRefCache",    defaultConfig().entryTtl(LONG));
